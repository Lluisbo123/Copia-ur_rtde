...
public:
	...
	enum eMoveMode
	{
		...
		WAIT_FINISHED//!< returns if the move finished, that means if an object is gripped or if requested target position is reached
	};
	...
	enum eUnit
	{
		...
		UNIT_NORMALIZED, ///< normalized value in the range 0.0 - 1.0, for position 0.0 means fully closed and 1.0 means fully open
	...
	enum eFaultCode
	{
		...
		FAULT_ACTION_DELAYED = 0x05, ///< Action delayed; the activation (re-activation) must be completed prior to perform the action.
		FAULT_ACTIVATION_BIT = 0x07, ///< The activation bit must be set prior to performing the action.

		// Minor faults (solid red LED)
		FAULT_TEMPERATURE = 0x08, ///< Maximum operating temperature exceeded (≥ 85 °C internally); let cool down (below 80 °C).
